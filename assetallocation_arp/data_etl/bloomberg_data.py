...
class Bloomberg:
    ...
    def referenceRequest(self, securities, fields, overrides={}, **kwargs):
            ...
            if msg.hasElement('securityData'):
                ...
                for securityData in securitiesData.values():
                    ...
                    if securityData.hasElement('securityError'):
                        ...
                        data.loc[data['bbergsymbol'] == security, 'bbergvalue'] = securityError.getElementValue(
                            'subcategory')
                        data.loc[data['bbergsymbol'] == security, 'status'] = "ERROR - {0} - {1}".format(
                            securityError.getElementValue('message'), timerequest)
                    else:
                        ...
                        for each in fieldData.elements():
                            ...
                            if each.isArray():
                                ...
                                for BulkArray in each.values():
                                    ...
                                    data_bulk = data_bulk.append(pd.DataFrame([values], columns=columns),
                                                                 ignore_index=True)
        ...
        data = data.append(bbg_data, ignore_index=True).drop_duplicates(['bbergsymbol', 'bbergfield'], keep='last')
    ...
    def historicalRequest(self, securities, fields, startdate, enddate, overrides={}, **kwargs):
            ...
            if msg.hasElement('securityData'):
                ...
                if securityData.hasElement('securityError'):
                    ...
                    for fld in fieldData.values():
                        ...
                        for v in [fld.getElement(i) for i in range(fld.numElements()) if
                                  fld.getElement(i).name() != 'date']:
        ...
        bbg_data = pd.DataFrame(bbg_data, columns=data.columns)
        data.loc[~data['bbergsymbol'].isin(
            bbg_data['bbergsymbol'].unique()), 'status'] = "ERROR - Symbol not returned - {0}".format(timerequest)
        data = data[data['status'].notnull()]
        data = data.append(bbg_data, ignore_index=True)
            ...
            for msg in response:
                ...
                if msg.hasElement('barData'):
                    ...
                    if msg.hasElement('responseError'):
                        ...
                        data.loc[data['bbergsymbol'] == bbergsymbol, 'bbergvalue'] = securityError.getElementValue(
                            'subcategory')
                        data.loc[data['bbergsymbol'] == bbergsymbol, 'status'] = "ERROR - {0} - {1}".format(
                            securityError.getElementValue('message'), timerequest)
    ...
    def referenceRequest2(self, df, securities, fields, overrides={}, **kwargs):
        ...
        for field in request['bbergfield'].unique():
            ...
            data = data.append(self.referenceRequest(field_securities, field, overrides, *kwargs))
    ...
    def historicalRequest2(self, df, securities, fields, datearray, overrides={}, **kwargs):
        ...
        for date in request['bbergdate'].dt.date.unique():
            ...
            for field in date_securitiesandfields['bbergfield'].unique():
                field_securities = date_securitiesandfields[date_securitiesandfields['bbergfield'] == field][
                    'bbergsymbol'].unique()
                data = data.append(self.historicalRequest(field_securities, field, date, date, overrides, *kwargs))
        return data
