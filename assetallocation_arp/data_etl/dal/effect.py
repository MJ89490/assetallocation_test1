...
class Effect(Strategy):
    ...
    def __init__(self, user: User, carry_type: str, closing_threshold: float, cost: float, day_of_week: int,
                 frequency: str, include_shorts_flag: bool, inflation_lag: int, interest_rate_cut_off_long: float,
        ...
        super().__init__(self.name, user, description)
    ...
    def insert(self, db: Db):
        self._version = db.call_proc('insert_effect_strategy',
                                     [self.name, self.description, self.user.user_id, self.carry_type,
                                      self.closing_threshold, self.cost, self.day_of_week, self.frequency,
                                      self.include_shorts_flag, self.inflation_lag, self.interest_rate_cut_off_long,
                                      self.interest_rate_cut_off_short, self.moving_average_long_term,
                                      self.moving_average_short_term, self.realtime_inflation_forecast_flag,
