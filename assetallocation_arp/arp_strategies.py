...
def run_model(model_type, mat_file, input_file, model_date=None):
    ...
    if model_type == Name.times.name:
        ...
        # run strategy
        signals, returns, r,  positioning, sig_chart = times.format_data_and_calc(times_inputs, asset_inputs, all_data)
        # write results to output sheet
        write_output_to_excel({Name.times.name: (asset_inputs,  positioning, r, signals, sig_chart, times_inputs)}, input_file)
...
def write_output_to_excel(model_outputs, input_file):
    ...
    if Name.times.name in model_outputs.keys():
        ...
        asset_inputs, positioning, returns, signals, sig_chart, times_inputs = model_outputs['times']
        ...
        sheet_times_output.range('rng_times_output').offset(-1, 3 * n_columns + 4).value = "TIMES S-Curve"
        sheet_times_output.range('rng_times_output').offset(0, 3 * n_columns + 4).value = sig_chart
