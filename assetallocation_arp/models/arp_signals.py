...
def momentum(index_data, inputs, week_day):
    sig = pd.DataFrame()
    sig_chart = pd.DataFrame(index=['raw', 's_curve'])
    for column in index_data:
        ...
        # storing last raw observation for charting purposes
        sig_chart.loc['raw', column] = sig[column][-1:].values
    ...
    # storing last observation for charting purposes
    sig_chart.loc['s_curve', :] = sig.iloc[-1:].values
    return sig, sig_chart.T
