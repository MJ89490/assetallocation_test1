...
def format_data(fica_inputs, asset_inputs, all_data):
    ...
    curve = all_data[asset_inputs_t[country[1]][ini:ini + 14]]
    #
...
def signals_and_returns(fica_inputs, carry_roll, country_returns):
    ...
    return carry_roll, signals, cum_cntr, returns


def daily_attribution(fica_inputs, asset_inputs, all_data, signal):
    ...
    n = len(all_data)
    asset_inputs_t = asset_inputs.set_index('country').T
    # creating carry & return dataframe for first country and then loop the rest plus 3x futures tickers
    futures = all_data[asset_inputs_t.loc['future_ticker', ['EUR', 'GBP', 'USD']]]
    curve_ox = all_data[asset_inputs_t[country[1]][28:32]]
    for i in range(2, m + 1):
    ...
    #
    for k in range(1, n):
        return_daily.iloc[k, m + 1] = return_daily.iloc[k - 1, m + 1] * \
